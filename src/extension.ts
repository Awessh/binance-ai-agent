...
import { VscodeMcpBinanceClient } from "./mcp/client";
import { VscodeLlmProvider } from "./llm/provider";
import { HostMessage, OrderRequest, WebviewMessage } from "./shared/types";

export function activate(context: vscode.ExtensionContext): void {
  context.subscriptions.push(
    vscode.commands.registerCommand("binanceMcpDashboard.open", () => {
...
class BinanceDashboardPanel {
  private static currentPanel: BinanceDashboardPanel | undefined;
  private readonly panel: vscode.WebviewPanel;
  private readonly client = new VscodeMcpBinanceClient();
  private readonly llm = new VscodeLlmProvider();
  private readonly disposables: vscode.Disposable[] = [];
  private pendingOrder: OrderRequest | undefined;
  private latestSnapshot: Awaited<ReturnType<VscodeMcpBinanceClient["getSnapshot"]>> | undefined;
  ...
  private constructor(private readonly context: vscode.ExtensionContext) {
    ...
    this.panel.webview.onDidReceiveMessage(
      (message: HostMessage) => void this.handleMessage(message),
      undefined,
      this.disposables
    );
  ...
  private async getSnapshotForRequest(): Promise<Awaited<ReturnType<VscodeMcpBinanceClient["getSnapshot"]>>> {
